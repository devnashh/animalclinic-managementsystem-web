...
$vet = $_SESSION['username'] ?? null; // Get logged-in admin/vet username
...
$query = "SELECT 
            ...
            COALESCE(p.type, b.pet_type, 'N/A') AS pet_type
          FROM bookings b
          LEFT JOIN client c ON b.client_id = c.id
          LEFT JOIN pets p ON b.pet_id = p.id
          WHERE b.service_type IN ('Consultation', 'Vaccination', 'Surgery')"; // Filter for vet services only
...
if (!empty($search_query)) {
    $query .= " AND (c.full_name LIKE ? OR b.walkin_customer_name LIKE ?)";
    $params[] = '%' . $search_query . '%';
    $params[] = '%' . $search_query . '%';
    $types .= "ss";
}
...
// Apply filter
if ($filter == 'walk-in') {
    $query .= " AND b.booking_type = 'walk-in'";
} elseif ($filter == 'online') {
    $query .= " AND b.booking_type = 'online'";
}
...
if ($filter == 'latest') {
    ...
    $query .= " ORDER BY b.created_at DESC"; // Default sorting
}
        ...
        <table class="table table-bordered table-hover">
            <thead class="table-primary">
                <tr>
                    ...
                    <th>Status</th>
                    <th>Booking Type</th> <!-- Added booking type column -->
                    <th>Actions</th>
                </tr>
            </thead>
            <tbody>
        ...
        <script>
            document.querySelectorAll('.update-status').forEach(button => {
                button.addEventListener('click', function () {
                    ...
                    // Prompt for a reason if status is Rejected
                    if (newStatus === 'Rejected') {
                        var reason = prompt("Please enter the reason for rejection (optional):");
                        // Allow empty reason, backend will handle the default message
            ...
            document.querySelectorAll('.archive-booking').forEach(button => {
                button.addEventListener('click', function () {
                    const bookingId = this.getAttribute('data-id');
