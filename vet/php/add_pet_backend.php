...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $query = "INSERT INTO pets (client_id, name, age, sex, color, type, breed, profile_picture, weight, owner_name, owner_contact, owner_address, owner_email)
              VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

    // Prepare and bind parameters to prevent SQL injection
    if ($stmt = $conn->prepare($query)) {
        $stmt->bind_param('issssssssssss', $client_id, $name, $age, $sex, $color, $type, $breed, $profile_picture, $weight, $owner_name, $owner_contact, $owner_address, $owner_email);
