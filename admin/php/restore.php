...
if (isset($_GET['id'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        if ($insertStmt->execute()) {
            ...
            header("Location: ../settings.php?success_user=1");
            exit;
        } else {
            header("Location: ../settings.php?error_user=1");
            exit;
        }
    } else {
        header("Location: ../settings.php?error_user=2"); // User not found in archive
        exit;
    }
} else {
    header("Location: ../settings.php?error_user=3"); // No ID provided
    exit;
}
?>