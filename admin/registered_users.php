...
<body class="d-flex">
    ...
    <div class="flex-grow-1 p-4" style="font-family: 'Nunito', sans-serif;">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="role" name="role" required>
                                    ...
                                    <option value="vet" <?php echo isset($_SESSION['form_data']['role']) && $_SESSION['form_data']['role'] == 'vet' ? 'selected' : ''; ?>>Veterinarian
                                    </option>
        ...
        if (isset($_GET['message']) || isset($_GET['error'])) {
            ...
            echo "<div id='alert-box' class='alert mx-3 mb-4 $alertType' style='display:none'>$alertMessage</div>";
            ?>
            <script>
                setTimeout(function() {
                ...
                <tbody>
                    ...
                    if (mysqli_num_rows($result) > 0) {
                        while ($row = mysqli_fetch_assoc($result)) {
                            ...
                            echo "<td>
                                        <button type='button' class='btn btn-primary btn-sm bi bi-pencil-square' data-bs-toggle='modal'
                                    data-bs-target='#editModal'
                                    data-id='" . htmlspecialchars($row['id']) . "'
                                    data-username='" . htmlspecialchars($row['username']) . "'
                                    data-email='" . htmlspecialchars($row['email']) . "'
                                    data-role='" . htmlspecialchars($row['role']) . "'></button>

                            <form method='POST' action='php/archive_users.php' class='d-inline'>
                                <input type='hidden' name='user_id' value='" . htmlspecialchars($row['id']) . "'>
                                <button type='submit' class='btn btn-danger btn-sm bi bi-archive' onclick='return confirm(\"Are you sure you want to archive this user?\");'></button>
                            </form>
                                    </td>";
                            echo "</tr>";
                        }
